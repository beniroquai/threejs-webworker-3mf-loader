...
// Below: the standard THREE.js 3mf loader code, with our monkeypatch
// methods scattered as appropriate.
	...
	parse(data) {
		...
		function loadDocument(data) {
			...
			for (let i = 0; i < modelPartNames.length; i++) {
				...
				monkeyPatchQuerySelectors(xmlData);
		...
		function parseRelsXml(relsFileText) {
			...
			monkeyPatchQuerySelectors(relsXmlData);
		...
		function parseBasematerialsNode(basematerialsNode) {
			monkeyPatchQuerySelectors(basematerialsNode);
		...
		function parseTextures2DGroupNode(texture2DGroupNode) {
			monkeyPatchQuerySelectors(texture2DGroupNode);
		...
		function parseColorGroupNode(colorGroupNode) {
			monkeyPatchQuerySelectors(colorGroupNode);
		...
		function parseMetallicDisplaypropertiesNode(metallicDisplaypropetiesNode) {
			monkeyPatchQuerySelectors(metallicDisplaypropetiesNode);
		...
		function parseMeshNode(meshNode) {
			monkeyPatchQuerySelectors(meshNode);
		...
		function parseComponentsNode(componentsNode) {
			monkeyPatchQuerySelectors(componentsNode);
		...
		function parseObjectNode(objectNode) {
			monkeyPatchQuerySelectors(objectNode);
		...
		function parseResourcesNode(resourcesNode) {
			monkeyPatchQuerySelectors(resourcesNode);
		...
		function parseBuildNode(buildNode) {
			monkeyPatchQuerySelectors(buildNode);
		...
		function parseModelNode(modelNode) {
			monkeyPatchQuerySelectors(modelNode);
